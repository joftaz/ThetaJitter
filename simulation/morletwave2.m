function [wvlt_amp, wvlt_phase]  = morletwave2(frequencies,c,signal,fs,bl_range,varargin)
% wvlt  = morletwave(frequencies,c,signal,fs,bc)
...
% 'normalize' followed by 'on' or 'off' - determines whether Hilbert-compatible
...
% Revised by Edden Gerber 2010-2013
%
...
% wavelet, leading to an output amplitude equivalent to that of the Hilbert
% transform. 
...
% Edden Gerber 19.5.2013: Made computation of the phase matrix dependent on the existence of the
% second output variable (reduce memory and run time if not requested). 
%
...
%     disp(['trial ' num2str(trial) ' out of ' num2str(trials)]);
    ...
    for freq = (1:Nf)
        ...
        if UseGpu
            ...
            wvlt_trial = fconv(morlet(freq,wlim(freq,1):wlim(freq,2)),signal(trial,:));
