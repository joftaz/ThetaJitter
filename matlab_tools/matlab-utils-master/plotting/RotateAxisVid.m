classdef RotateAxisVid
    
    methods(Static)
        function sph = getCamPosSph(axh)
            if nargin < 1
                axh = gca;
            end
            
            pos = axh.CameraPosition;
            posTarget = axh.CameraTarget;
            [sph(1), sph(2), sph(3)] = cart2sph(pos(1), pos(2), pos(3))
        end
    end
end
%         
%         function makeVideo(ViewZ, FileName,OptionZ)
