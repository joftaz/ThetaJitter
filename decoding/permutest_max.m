function [p_value_mask, t_value_matrix, permutation_distribution_max, permutation_distribution_min] = permutest_max( trial_group_1, trial_group_2, dependent_samples, ...
...
% two-sided test. 
% * The number of permutations (nP) sets a minimal boundary for the resulting
...
% * When running a test for independent samples, there can be an unequal 
% number of trials in each of the two groups. The same amount of trials for
% each condition will be preserved in the randomized permutations, since
% this parameter is a property of the experiment and not an exchangeable
% attribute (for example, if the main comparison is between a small sample
% and a large sample, then the permutation distribution should all be based
% on cases of comparing equally small and large samples). 
%
% Syntax: 
% [clusters, p_values, t_sums, permutation_distribution ] = PERMUTEST(x,y) 
% runs a cluster-based permutation based for dependent samples, searching
% for clusters where x is larger than y. x and y should be 2D or 3D matrices, 
% where the last dimension is the dimension across which the test variance 
% is defined (that is, trials or subjects). clusters is a cell array with 
% each cell holding the vector/matrix indexes corresponding to a specific 
...
% t-value for the purpose of this test. Default value is 0.05. 
...
% trials and test type. Default value is the smaller of 10^4 or largest 
% possible number given number of trials or subjects. 
...
% 1.6.2017 Edden: When testing multiple clusters, all of the cluster t-sum
% values are compared against the maximum-value null distribution - not
% against the distribution of 2nd largest, 3rd largest values etc.. This is
% in line with the recommendation in the Maris and Oostenveld paper. 
% 1.6.2017 Edden: Default number of clusters is now "inf" and not 3, since
% now the test controls for the FA across all clusters
% 1.6.2017 Edden: Removed the option to use the GetClusters function - it 
% is assumed that image processing toolbox is always supported. 
% 1.6.2017 Edden: Added support for two-sided tests. 
% 4.6.2017 Edden: Added support for independent samples and corrected some
% bugs related to testing 3D data. THIS VERSION IS NO LONGER BACKWARD
% COMPARIBLE SINCE THE INPUT STRUCTURE HAS CHANGED. 
% 6.6.2017 Edden: "clusters" & "p_values" output variables are now
% initialized as size 0 instead of size 1, to avoid returning a non-empty
% array when no clusters were found. 
...
if isempty(trial_group_2)
    trial_group_2 = zeros(size(trial_group_1));
    empty_second = true;
end
...
%         t_value_vector = simpleTTest(trial_group_1'-trial_group_2',0);
         t_value_vector = simpleTTest(trial_group_1',0.5);
    else
...
permutation_distribution_max = zeros(num_permutations,1);
permutation_distribution_min = permutation_distribution_max;
for p = 1:num_permutations
    if dependent_samples
        ...
        if num_data_points_y == 1 
            ...
            % two-dimensional trial data
%             D = trial_group_1-trial_group_2;
            D = trial_group_1;
            D = bsxfun(@times,D,permute(permutation_vectors(:,p), [3 2 1]));
%             t_value_vector = simpleTTest(D,0);
            t_value_vector = simpleTTest(D,0.5);
        end
    else
...
%     end
    permutation_distribution_max(p) = max(t_value_vector(:));
    permutation_distribution_min(p) = min(t_value_vector(:));
end
...
% t_value_matrix = simpleTTest(trial_group_1-trial_group_2,0);
t_value_matrix = simpleTTest(trial_group_1,0.5);
p_value_mask = t_value_matrix > tThreshold;
...
xmean1 = sum(x1)/n1; % works faster than mean
xmean2 = sum(x2)/n2; % works faster than mean
